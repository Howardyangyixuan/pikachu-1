...
var run = function run() {
  ...
  text.innerText = _css.default.substr(0, n);
  text.scrollTop = text.scrollHeight;
  css.innerText = _css.default.substr(0, n);
  n++;
}; // let id = setInterval(run, time);


var speedAdjust = function speedAdjust(newTime) {
  window.clearInterval(id);
  time = newTime;
...
var play = function play() {
  window.clearInterval(id);
  id = setInterval(run, time); // return id
};

var id = setInterval(run, time); // let id = play();

btnPause.onclick = pause;
btnPlay.onclick = play;
btnSlow.onclick = slow; //   speedAdjust(100);

btnNormal.onclick = normal; // speedAdjust(50);

btnFast.onclick = fast; //   speedAdjust(0);
